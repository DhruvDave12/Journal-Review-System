...
const Review = () => {
  ...
  const handleOnAcceptArticle = async (articleID) => {
    try {
      ...
      const response = await axiosInstance.post(
        `/review/requests/accept/${articleID}`
      );
      if (response.status === 200) {
        console.log("REQUEST ACCEPTED SUCCESSFULLY");
        showToast("Article Successfully accepted for review.", "success");
      } else {
  ...
  const handleOnRejectArticle = async (articleID) => {
    try {
      setLoading(true);
      const response = await axiosInstance.post(
        `review/requests/reject/${articleID}`
      );
      if (response.status === 200) {
    ...
    <div className={styles.rev}>
      <div className={styles.seclink}>
        ...
        <div className={styles.currjour}>
          <p onClick={() => setNewJournalToggle(false)}>Currently Reviewing</p>
          <div
            style={{
              height: !newJournalToggle ? "6px" : "2px",
              width: "100%",
      ...
      {newJournalToggle ? (
        <NewJournals
          requests={newReviewRequests}
          onAccept={handleOnAcceptArticle}
          onReject={handleOnRejectArticle}
        />
      ) : (
